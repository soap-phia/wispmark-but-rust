...
fn main() {
    ...
    fs::copy(&server_package, out_dir.join("server-package.json")).expect("Failed to copy server package.json");
    fs::copy(&server_mjs, out_dir.join("server.mjs")).expect("Failed to copy server.mjs");
    fs::copy(&client_package, out_dir.join("client-package.json")).expect("Failed to copy client package.json");
